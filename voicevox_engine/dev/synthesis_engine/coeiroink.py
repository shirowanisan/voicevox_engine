...
class EspnetModel:
    ...
    def get_tsukuyomichan_model(cls, use_gpu):
        acoustic_model_folder_path = "./models/TSUKUYOMICHAN_COEIROINK_MODEL_v.0.0.1/ACOUSTIC_MODEL"
    ...
    def get_harumachi_model(cls, use_gpu):
        acoustic_model_folder_path = "./models/HARUMACHI_COEIROINK_MODEL_v.0.0.1/ACOUSTIC_MODEL"
        return cls.get_espnet_model(
            acoustic_model_path=f"{acoustic_model_folder_path}/100epoch.pth",
            acoustic_model_config_path=f"{acoustic_model_folder_path}/config.yaml",
            use_gpu=use_gpu,
    ...
    def get_ofutonp_model(cls, use_gpu):
        acoustic_model_folder_path = "./models/OFUTONP_COEIROINK_MODEL_v.0.0.1/ACOUSTIC_MODEL"
        return cls.get_espnet_model(
            acoustic_model_path=f"{acoustic_model_folder_path}/100epoch.pth",
            acoustic_model_config_path=f"{acoustic_model_folder_path}/config.yaml",
            use_gpu=use_gpu,
...
class MockSynthesisEngine(SynthesisEngineBase):
    ...
    def synthesis(self, query: AudioQuery, speaker_id: int, text: str = '') -> np.ndarray:
        ...
        wave = self.speaker_models[speaker_index].make_voice(tokens)
